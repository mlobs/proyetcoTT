...
mod = pd.read_json(r'C:\Users\Nicolás\Desktop\TT\ProyectoTT\proyetcoTT\resultados_df.json')
st.write(mod)

#*  Mostrando los Modelos en pantalla
with st.container():
    with open(r'C:\Users\Nicolás\Desktop\TT\ProyectoTT\proyetcoTT\modelos.json', 'r') as file:
...
with open(r'C:\Users\Nicolás\Desktop\TT\ProyectoTT\proyetcoTT\columnas_ejemplo.json', 'r') as file:
...
# Botón para guardar los datos y ejecutar el backend
    if st.button("Enviar"):
        ...
        with open(r"C:\Users\Nicolás\Desktop\TT\ProyectoTT\proyetcoTT\valor.json", "w") as file:
            json.dump(entrada, file)
        st.success("Datos enviados al backend")

        # Iniciar el backend
        backend_process = subprocess.Popen(["python", "proyetcoTT/modelo.py"])
